...
std::string wstr_to_str1(const std::wstring& arg)
{
...
std::wstring str_to_wstr1(const std::string& arg)
{
...
void CIniFileA::Load( std::istream& input , bool bMerge )
{
    ...
    while( std::getline( input , sRead ) )
    {
        ...
        if( !sRead.empty() )
        {
            ...
            case SECTION:
#ifdef _CINIFILE_DEBUG
                stream <<  "Parsing: Secton - " << sRead << std::endl;
				str = stream.str();
				chr = str.c_str();
				Log::Msg(chr);
#endif
            ...
            {
#ifdef _CINIFILE_DEBUG
                stream <<  "Parsing: Key - " << sRead << std::endl;
				str = stream.str();
				chr = str.c_str();
				Log::Msg(chr);
#endif
                ...
                if( pSection )
                {
                    ...
                    {
#ifdef _CINIFILE_DEBUG
                        stream <<  "Parsing: Key Value - " << sValue << std::endl;
						str = stream.str();
						chr = str.c_str();
						Log::Msg(chr);
#endif
            ...
            case COMMENT:
#ifdef _CINIFILE_DEBUG
                stream <<  "Parsing: Comment - " << sRead << std::endl;
				str = stream.str();
				chr = str.c_str();
				Log::Msg(chr);
#endif
            ...
            case OTHER:
#ifdef _CINIFILE_DEBUG
                stream <<  "Parsing: Other - " << sRead << std::endl;
				str = stream.str();
				chr = str.c_str();
				Log::Msg(chr);
#endif
...
#ifdef _CINIFILE_DEBUG
	std::ostringstream stream;
    stream <<  "CIniFileA::Load() - " << fileName << std::endl;
...
#ifndef _WIN32
    output.open( wstr_to_str1(fileName).c_str() , std::ios::binary );
#else
...
#ifndef _WIN32
    input.open( wstr_to_str1(fileName).c_str() , std::ios::binary );
#else
#if defined(_MSC_VER) && (_MSC_VER >= 1200) && (_MSC_VER < 1300)
    input.open( wstr_to_str1(fileName).c_str() , std::ios::binary );
#elif defined(_MSC_VER) && (_MSC_VER >= 1300)
