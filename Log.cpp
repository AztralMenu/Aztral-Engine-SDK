...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(szTimestamp, "[%02d:%02d:%02d] %%s\n"_Secure, current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
		sprintf_s(chLogBuff, szTimestamp, "INIT: Aztral Menu"_Secure);
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "Aztral-menu.log"_Secure);
			...
			OutputDebugStringA("INIT: PotatoHook"_Secure);
		}
		else {
			useFileOutput = false;
		}
...
void Log::Error(const char* fmt, ...) {
	...
	MessageBoxA(NULL, chLogBuff, "ERROR"_Secure, MB_ICONERROR);
}
...
void Log::Fatal(const char* fmt, ...) {
	...
	MessageBoxA(NULL, chLogBuff, "FATAL ERROR"_Secure, MB_ICONERROR);
